...
Общее количество токенов ENQ (ERC-20) составило 126,880,563.75 ENQ.
...
### Модель эмиссии майнинга

Общее количество ENQ, доступное для майнинга, и вознаграждения по реферальной программе составят 160,119,436.25 ENQ в течение 25 лет с момента запуска основной сети.
Данные ENQ будут распределены среди узлов PoW, PoS и PoA, которые будут поддерживать работу сети и участвовать в формировании цепочки блоков. Блоки будут создаваться каждые 15 секунд в течении последующих 25 лет.

Таблица генерации блоков и эмиссии новых монет:
...
Общее вознаграждение за блок будет составлять 3,046 ENQ, которое будет распределяться между узлами сети следующим образом.
...
Распределение вознаграждения между узлами сети во времени представлен ниже.
...
Для участия в PoA-майнинге необходимо иметь активный баланс от 25 до 25,000.00 ENQ.  А также запустить PoA узел который должен иметь бесперебойное подключение к сети интернет. 
...
Для того чтобы стать PoS-валидатором, необходимо иметь баланс ENQ, входящий в топ 100 кошельков — рейтинг кошельков с наибольшим значением стейка —  но не менее 25,001.00 ENQ. Также необходимо создать PoS контракт в кошельке, чтобы получить PoS ID и запустить сервер для участия в создании цепочки блоков, который должен иметь бесперебойное подключение к сети интернет.

Любой из участников сети  может стать делегатором. Для этого необходимо отправить на адрес PoS ID контракта валидатора не менее 1 ENQ. Отправленные средства будут участвовать в общем стейке и получать ежедневное вознаграждение в ENQ. При этом делегатору нет необходимости находиться в постоянном подключении к сети интернет.
...
Валидаторы и делегаторы также будут участвовать в верификации транзакций и будут получать дополнительное вознаграждение от системы. Экономика этого процесса описана здесь.
...
Alice выпустила свой пользовательский токен Alice Token и определила пользовательскую комиссию в 1 AT. Это значит, что при каждой пересылке AT Alice будет получать на адрес Token ID (адрес токена) 1 AT. В этот момент автоматически произойдет списание с адреса AT комиссии за проведение этой транзакции в размере 0.1 ENQ.
